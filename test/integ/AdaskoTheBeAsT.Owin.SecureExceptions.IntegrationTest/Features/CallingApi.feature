...
Scenario: Calling non existent api with failure result and sanitized error message
    ...
    Then I should get error with message '{"Message":"No HTTP resource was found that matches the request URI"}'
...
Examples:
    | sampleUrl                                                               | expected                                                              |
    | /api/values?query=<script>alert('xss')</script>                         | {"Message":"No HTTP resource was found that matches the request URI"} |
    | /api/values/%2522%253e%253cscript%253ealert('xss')%253c%252fscript%253e | "Error"                                                               |
    | /api/values?query="><script>alert('xss')</script>                       | {"Message":"No HTTP resource was found that matches the request URI"} |
    | /api/values?query=%22onmouseover%3d%22alert('xss')%22                   | {"Message":"No HTTP resource was found that matches the request URI"} |
    | /api/values?query=';DROP TABLE Users;--                                 | {"Message":"No HTTP resource was found that matches the request URI"} |
